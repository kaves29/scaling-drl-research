...
from scale_rl.agents.saliency_sparse import create_saliency_mask
...
def _init_sac_networks(
    ...
    if cfg.actor_sparsity > 0.0 and not cfg.saliency_based_pruning:
        actor_sparse = True
        actor_saliency_based = False
    elif cfg.actor_sparsity > 0.0 and cfg.saliency_based_pruning:
        actor_sparse = False
        actor_saliency_based = True
    else:
       actor_sparse = False
       actor_saliency_based = False
    ...
    elif actor_saliency_based:
        actor_network_mask = create_saliency_mask(fake_actor_params, "actor", actor_network_def, cfg)
    else:
    ...
    actor = Trainer.create(
        ...
        sparse = actor_sparse if actor_sparse else actor_saliency_based,
    ...
    if cfg.critic_sparsity > 0.0 and not cfg.saliency_based_pruning:
        critic_sparse = True
        critic_saliency_based = False
    elif cfg.critic_sparsity > 0.0 and cfg.saliency_based_pruning:
        critic_sparse = False
        critic_saliency_based = True
    else:
       critic_sparse = False
       critic_saliency_based = False
    if critic_sparse:
    ...
    elif critic_saliency_based:
        critic_network_mask = create_saliency_mask(fake_critic_params, "critic", critic_network_def, cfg)
    else:
    ...
    critic = Trainer.create(
        ...
        dynamic_scale=dynamic_scale.DynamicScale() if cfg.mixed_precision else None,
        sparse = critic_sparse if critic_sparse else critic_saliency_based,
