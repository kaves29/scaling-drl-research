...
class TestSaveFrozenAgentSnapshotCheckpointDirIsAlwaysAbsolute(unittest.TestCase):
    ...
    def test_already_absolute_root_is_left_correct(self):
        ...
        # os.path.realpath() on both sides (not a plain startswith()): on
        # macOS, tempfile.mkdtemp() often returns a path under a symlinked
        # prefix (e.g. /var/folders/... -> /private/var/folders/...), and
        # .resolve() (used inside save_frozen_agent_snapshot) correctly
        # follows that symlink - so the resolved call path is still
        # "under" abs_root in the real filesystem sense, just not as an
        # unresolved string prefix.
        self.assertTrue(
            os.path.realpath(agent.save_checkpoint_called_with).startswith(os.path.realpath(abs_root))
        )
