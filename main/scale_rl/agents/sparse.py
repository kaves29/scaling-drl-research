...
def create_random_mask(params, sparsities, rng):
    params_unfrozen = unfreeze(params)
    flat_params = flatten_dict(params_unfrozen, sep='/')

    new_flat_masks = {}
    current_rng = rng
    for param_path, param_value in flat_params.items():
        if isinstance(param_path, tuple):
            param_name = '/'.join(param_path)
        else:
            param_name = param_path

        param_sparsity = sparsities.get(param_name, 0.0)
        keep_prob = 1.0 - param_sparsity
        current_rng, subkey = jax.random.split(current_rng)
        mask = jax.random.bernoulli(subkey, p=keep_prob, shape=param_value.shape)
        new_flat_masks[param_path] = mask
