...
def get_feature_norm(activations) -> dict:
    """
    Computes the feature norm for a single layer of activations.

    Args:
        activations (array-like): The activations of the layer.
        prefix (str): A prefix string to namespace the norm keys.

    Returns:
        dict: A dictionary containing the feature norm for the layer and the total norm.
    """

    activs = jnp.array(activations)
    
    # If activations have more than 2 dimensions, reshape them to combine batch dimensions
    if activs.ndim > 2:
        activs = activs.reshape(-1, activs.shape[-1])

    # Compute the L2 norm for each example in the batch
    batch_norms = jnp.linalg.norm(activs, ord=2, axis=-1)

    # Compute the mean L2 norm across the batch
...
def print_num_parameters(
    input_dict_list: Sequence[FrozenDict], network_type: str
) -> int:
    """
    Return number of trainable parameters
    """
    total_params = 0
    for sub_layer_name, params in list(input_dict_list.items()):
        total_params += np.prod(jnp.array(params).shape)
    return total_params
...
def get_intermediates_dict(intermediates):
    newintermediates = {}
    del intermediates['intermediates']['__call__']
    for category, layers in intermediates['intermediates'].items():
        for layer_name, layer_details in layers.items():
            if layer_name == '__call__':
                continue
            new_key = f"{category}_{layer_name}"
            traced_obj = layer_details.get('__call__')
            jnp_array = jnp.array(traced_obj)
            newintermediates[new_key] = jnp_array
