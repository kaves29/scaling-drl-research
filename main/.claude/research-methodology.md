...
Null distribution: the null must mirror the primary analysis's structure exactly, not just involve "a healthy critic." Select two independently-trained default-architecture SimBa agents, A and B (different seeds; neither critic has trained on the other's actor's data — essential, not optional). Using π_A held fixed, compute g_{A|A} = ∇_θJ(π_A; Q_A) (real signal, A's own critic) and g_{A|B} = ∇_θJ(π_A; Q_B) (foreign healthy critic swapped in), then compute D_dir, D_mag, and D_grad on this pair exactly as in the primary analysis — never a raw comparison between two healthy critics' outputs. This isolates the distortion caused by ordinary "foreign critic" unfamiliarity alone, with no pathology involved on either side. Repeat across multiple independent healthy A/B pairs to build a real distribution, not a single point estimate — as of 2026-09-08, all C(10,2)=45 unique pairs from the shared baseline-calibration pool (see the Null Baseline section below), not a caller-selected seed subset (the null_seeds config field was removed accordingly). Compare the degraded-critic distortion (g_{D|D} vs g_{D|R}) against this null distribution using the (mean + 2σ) rule (implemented in experiments/angle_2b/statistics.py) — do not invent a new statistical threshold for this step. This is a DIFFERENT rule from Angle 1's onset/propagation threshold (a 95th percentile — see the 2026-08-28 correction in Onset Definitions above), not "the same criterion established elsewhere in this study" as this section previously implied: (mean + 2σ) remains the appropriate choice here specifically because an empirical percentile from a pairwise-pseudo-replicated 45-point sample (each of the 10 underlying agents contributes to 9 pairs) is not obviously more reliable than one from the previous ≤5 independent points, and switching threshold rules was explicitly NOT authorized as part of the 2026-09-08 pool expansion — that remains a separate, pending decision. Do not silently make these two rules consistent with each other by changing either implementation without that explicit decision. State explicitly, as a limitation, that this comparison assumes the magnitude of ordinary foreign-critic unfamiliarity is comparable between a healthy-healthy pairing and a degraded-reference pairing — a standard assumption underlying null-baseline comparisons throughout this methodology, not a fully verified equivalence. Every reported comparison also flags results close to (not just beyond) the threshold as marginal (is_marginal, within 20% of the null_mean-to-threshold margin on either side) — see the Null Baseline section's Reporting safeguard.
...
Constructed from independently-trained, default-architecture (D2W512) critics — never a scaled critic. Run through the identical Angle 2A procedure to establish the natural degree of critic-to-critic divergence expected from ordinary training/seed variance alone, absent any pathology. Every reported divergence (E_D vs. E_R, and downstream in 2B/2C) must be interpreted relative to this null, not as a raw, unscaled number — a gap that falls within the null's natural spread is not evidence of degradation.

Source (refactored 2026-09-08): the shared baseline-calibration pool (see analysis/baseline_calibration_pool.py) — Angle 1's own 5 locked baseline seeds plus 5 additional dedicated calibration-pool agents, trained identically (same architecture, same procedure) but under a distinct experiment name/seed range so Angle 1's 150 locked runs are never touched, modified, or mixed with this pool. This is genuinely shared, one-time infrastructure: the same 10-agent pool per environment feeds Angle 2A's own null (below), Angle 2B/2C's null distribution (see Angle 2B's Null distribution paragraph above), and the N/W window calibration (see Onset Definitions below) — not three separately-capped constructions. Replaces the previous design, which trained a fresh healthy-vs-healthy pair per Angle 2A seed per matchup (capped at 5, one per seed) — that fresh-training approach existed only because no mechanism previously persisted Angle 1 agents in a form Angle 2A could evaluate directly; experiments/angle_1.py's save_probe_capture_snapshot opt-in flag now provides that, matching this section's original intent ("reusable, at no extra compute cost") more faithfully than the design it replaces.

Pairing (Angle 2A and Angle 2B/2C both): all C(10,2)=45 unique unordered pairs among the 10 pool agents (project decision, 2026-09-08), not a disjoint subset — see experiments/angle_2a/pool_null_baseline.py. Each pair contributes one E_A and one E_B diagonal-error value for Angle 2A's own null (90 values total, pooled — see that module's docstring for why E_A/E_B are not kept as separate D-side/R-side nulls), or one D_dir/D_mag/D_grad distortion value for Angle 2B/2C's null (45 values). Because every pool agent appears in 9 of the 45 pairs, these values are not mutually independent (pseudo-replication) — a deliberate, accepted tradeoff for a substantially larger empirical sample than the previous ≤5-point null. Computed once per environment and cached (results/angle_2a_pool_null/{environment}/ for Angle 2A's own null), not recomputed per seed.

Reporting safeguard (added 2026-09-08): every null-threshold comparison flags results close to (not just beyond) the threshold as marginal, independent of the null's size — see experiments/angle_2b/statistics.py's is_marginal (within 20% of the null_mean-to-threshold margin on either side). A larger null reduces sampling noise in the threshold estimate itself, but does not exempt a borderline observed value from being reported with appropriately reduced confidence.
...
Merging, mixing, or storing the shared baseline-calibration pool's 5 dedicated agents (see the Null Baseline section's Source paragraph) under the same experiment name/seed range/results path as Angle 1's own 5 locked baseline seeds, or extending analysis/baseline_calibration.py's 95th-percentile onset/propagation threshold to use the expanded 10-seed pool (that threshold stays hard-locked to Angle 1's original 5 - see Onset Definitions' N/W seed count note) — added 2026-09-08.
...
Whether Angle 2B/2C's null-threshold rule should move from (mean + 2σ) to an empirical percentile now that the null distribution has grown from ≤5 points to 45 (raised 2026-09-08, explicitly deferred - user instruction: "Do NOT change any threshold rule as part of this task... flagged as a separate, explicit decision for the user to make afterward"). Not resolved, not forgotten - the 45 values are pairwise-pseudo-replicated (each of the 10 pool agents appears in 9 pairs), so a larger raw count does not straightforwardly mean a more reliable percentile; this needs its own explicit evaluation, not an automatic switch. Raise this again the next time Angle 2B/2C's statistical methodology is discussed.